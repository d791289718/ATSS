...
def prepare_for_coco_rotated_detection(predictions, dataset):
    ...
    coco_results = []
    import mmcv
    t = mmcv.load('detections.pkl')
    
    for image_id, pr in tqdm(enumerate(t)):
        ...
        keep = py_cpu_nms_poly_fast(pr, 0.6)
        polys = pr[keep]
...
def bak_prepare_for_coco_rotated_detection(predictions, dataset):
    import pycocotools.mask as mask_util
...
def prepare_for_coco_detection(predictions, dataset, is_rotated):
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        if not is_rotated:
            ...
            boxes = prediction.get_bbox_xywh().tolist()
