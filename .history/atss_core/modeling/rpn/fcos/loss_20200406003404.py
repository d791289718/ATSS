...
class FCOSLossComputation(object):
    ...
    def prepare_targets(self, points, targets, rtargets, is_rotated=True):
        ...
        for level in range(len(points)):
            ...
            if self.norm_reg_targets:
                reg_targets_per_level[:, :4] = reg_targets_per_level[:, 4] / self.fpn_strides[level]
    ...
    def compute_rotated_targets_for_locations(self, locations, rtargets, object_sizes_of_interest):
        ...
        for im_i in range(len(rtargets)):
            ...
            # add the ang
            reg_targets_per_im = torch.stack([reg_targets_per_im, theta], dim=2)
    ...
    centerness, targets, rtargets, is_rotated):
        ...
        for l in range(len(labels)):
            ...
            angle_flatten.append(angle[l].reshape(-1))
        ...
        angle_flatten = torch.cat(angle_flatten, dim=0)
        ...
        if pos_inds.numel() > 0:
            ...
            angle_loss = self.angle_loss_func(
                angle_flatten,
                angle_targets_flatten,
            ) / num_pos_avg_per_gpu
